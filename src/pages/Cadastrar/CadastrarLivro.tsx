...
export function CadastrarLivro() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 flex flex-col items-center">
      <h1 className="text-3xl font-bold text-blue-700 mb-6">Livros Cadastrados</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl">
          {livroCadastrado.map((livro, index) => (
            <div key={index} className="bg-white rounded-xl shadow-lg p-4 flex flex-col items-center">
              ...
              {favoritar === true ? (
                <IoIosStarOutline className="m-2 mr-60 mb-5 cursor-pointer" size={20} color="red"/> ) : (
                  <IoIosStarOutline className="m-2 mr-60 mb-5 cursor-pointer" size={20} color="gray"/>
                ) 
                }
              ...
              <p className="text-sm text-gray-600 text-center">por {livro.autor}</p>
              <p className="text-sm text-gray-500">Páginas: {livro.paginas}</p>
              <button className="font-20 bg-blue-400 text-white font-bold w-40 h-10 p-2 mt-5 rounded-xl cursor-pointer">Cadastrar</button>
